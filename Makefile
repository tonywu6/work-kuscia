...
IMAGE_PAUSE = k8s.gcr.io/pause:3.6

.PHONY: image-arm64-pause
image-arm64-pause:
	docker pull --platform=linux/arm64 ${IMAGE_PAUSE}
	docker image save ${IMAGE_PAUSE} -o ./build/pause/pause-linux-arm64.tar

IMAGE_KUSCIA_ENVOY_ARM64 = tonywu6/kuscia-envoy:linux-arm64

.PHONY: image-arm64-kuscia-envoy
image-arm64-kuscia-envoy:
	docker build --platform=linux/arm64 \
		-f build/dockerfile/kuscia-envoy-arm64.Dockerfile \
		-t ${IMAGE_KUSCIA_ENVOY_ARM64} .

IMAGE_KUSCIA_ARM64 = tonywu6/kuscia:linux-arm64

.PHONY: image-arm64-kuscia
image-arm64-kuscia: image-arm64-kuscia-envoy
	docker build --platform=linux/arm64 \
		-f build/dockerfile/kuscia-arm64.Dockerfile \
		-t ${IMAGE_KUSCIA_ARM64} .

IMAGE_SECRETFLOW_ARM64 = tonywu6/secretflow:linux-arm64

.PHONY: image-arm64-secretflow
image-arm64-secretflow:
	docker build --platform=linux/arm64 \
		-f build/dockerfile/secretflow-arm64.Dockerfile \
		-t ${IMAGE_SECRETFLOW_ARM64} .
	docker image save --output ${CWD}/build/dockerfile/compose/public/secretflow-image.tar \
		${IMAGE_SECRETFLOW_ARM64}

.PHONY: image-arm64
image-arm64: image-arm64-pause image-arm64-kuscia-envoy image-arm64-kuscia image-arm64-secretflow

.PHONY: compose
compose:
	(cd build/dockerfile/compose && docker compose up --build)
