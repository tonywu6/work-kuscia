...
func Run(ctx context.Context, gwConfig *config.GatewayConfig, clients *kubeconfig.KubeClients, afterRegisterHook controller.AfterRegisterDomainHook) error {
	...
	if !masterConfig.Master {
		err = controller.RegisterDomain(ctx, gwConfig.DomainID, masterConfig.MasterProxy.Path, gwConfig.CsrData, prikey, afterRegisterHook)
		if err != nil {
		...
		for i := 1; i <= defaultHandshakeRetryCount; i++ {
			err = controller.HandshakeToMaster(ctx, gwConfig.DomainID, masterConfig.MasterProxy.Path, prikey)
			if err == nil {
				break
			}
